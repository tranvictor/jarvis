...
func TestFindNodeNameByURL(t *testing.T) {
	...
	name, ok = FindNodeNameByURL(nodes, "https://rpc.example.com/rpc/KEY")
	if !ok || name != "infra" {
		t.Fatalf("schemeless stored URL: got (%q, %v), want infra", name, ok)
	}
	name, ok = FindNodeNameByURL(map[string]string{"infra": "https://rpc.example.com/rpc/KEY"}, "rpc.example.com/rpc/KEY")
	if !ok || name != "infra" {
