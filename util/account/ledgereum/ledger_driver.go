...
func (w *ledgerDriver) SignTypedMessage(
	path accounts.DerivationPath,
	domainHash []byte,
	messageHash []byte,
) ([]byte, error) {
...
func (w *ledgerDriver) ledgerSign(
	...
	if tx.Type() == types.LegacyTxType {
		...
		// fmt.Printf("ledgerDriver is signing dynamic fee type\n")
		if txrlp, err = rlp.EncodeToBytes([]interface{}{
			chainID, tx.Nonce(), tx.GasTipCap(), tx.GasFeeCap(), tx.Gas(), tx.To(), tx.Value(), tx.Data(), tx.AccessList(),
		}); err != nil {
			return common.Address{}, nil, err
		}
	} else {
		return common.Address{}, nil, fmt.Errorf("ledger driver not support this tx type")
	}
	...
	for len(payload) > 0 {
		...
		// fmt.Printf("getting reply: %v\n", reply)
		// Shift the payload and ensure subsequent chunks are marked as such
		payload = payload[chunk:]
		op = ledgerP1ContTransactionData
	}

	// fmt.Printf("reply len: %d\n", len(reply))
	// fmt.Printf("signature len requirement: %d\n", crypto.SignatureLength)
...
//	signature R | 32 bytes
//	signature S | 32 bytes
func (w *ledgerDriver) ledgerSignTypedMessage(
	derivationPath []uint32,
	domainHash []byte,
	messageHash []byte,
) ([]byte, error) {
